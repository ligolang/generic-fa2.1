#import "../lib/contracts/fa2.1-single-asset.mligo" "FA2_1_single_asset"
...
let get_initial_storage (a, b, c : nat * nat * nat) = 
  ...
  let approvals = (Big_map.empty : FA2_1_single_asset.Approvals.t)
  in
  ...
  let metadata = FA2_1_single_asset.Metadata.init() in
...
let assert_balances 
  (contract_address : (FA2_1_single_asset.parameter, FA2_1_single_asset.storage) typed_address ) 
  ...
  let () = match (Big_map.find_opt owner1 ledger.values) with
    Some amt -> assert (amt = balance1)
  | None -> failwith "incorret address" 
  in
  let () = match (Big_map.find_opt owner2 ledger.values) with
    Some amt ->  assert (amt = balance2)
  | None -> failwith "incorret address" 
  in
  let () = match (Big_map.find_opt owner3 ledger.values) with
...
let originate_fa2_1 initial_storage =
  let (addr,_,_) = Test.originate_from_file "../lib/contracts/fa2.1-single-asset.mligo" "main" [] (Test.compile_value initial_storage) 0tez in
  (Test.cast_address addr : (FA2_1_single_asset.parameter, FA2_1_single_asset.storage) typed_address)
...
let test_atomic_transfer_failure_not_operator = 
  ...
  | Fail (Rejected (err, _))  -> assert (Test.michelson_equal err (Test.eval FA2_1_single_asset.Errors.not_operator))
...
let test_atomic_transfer_failure_not_suffient_balance = 
  ...
  match result with
    Success _ -> failwith "This test should fail"
  | Fail (Rejected (err, _))  -> assert (Test.michelson_equal err (Test.eval FA2_1_single_asset.Errors.ins_balance))
  | Fail _ -> failwith "invalid test failure"
...
let test_empty_transfer_and_balance_of = 
  ...
  let balance_of_requests = {
    requests = ([] : FA2_1_single_asset.Balance_of.request list);
  ...
  let _ = Test.transfer_to_contract_exn contr 
    (Update_operators [
      (Remove_operator {
        ...
        operator = op1;
        token_id = 0n;
      }:FA2_1_single_asset.Update.unit_update)
    ]) 0tez in
