...
let get_initial_storage (a, b, c : nat * nat * nat) = 
  ...
  let approvals = (Big_map.empty : FA2_multi_asset.Approvals.t)
  in
  ...
  let metadata = FA2_multi_asset.Metadata.init() in
...
let test_empty_transfer_and_balance_of = 
  ...
  let balance_of_requests = {
    requests = ([] : FA2_multi_asset.Balance_of.request list);
  ...
  let _ = Test.transfer_to_contract_exn contr 
    (Update_operators [
      (Remove_operator {
        ...
        operator = op1;
        token_id = 2n;
      }:FA2_multi_asset.Update.unit_update) 
    ]) 0tez in
